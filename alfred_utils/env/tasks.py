...
class BaseTask(object):
    ...
    def __init__(self, traj, env, args, reward_type='sparse', max_episode_length=2000, task_type_optional=None):
        # settings
        self.traj = traj
        self.env = env
        self.args = args
        if task_type_optional is None:
            self.task_type = self.traj['task_type']
        else:
            self.task_type =task_type_optional
...
def get_task(task_type, traj, env, args, reward_type='sparse', max_episode_length=2000, task_type_optional=None):
